/**
 * Search and Metrics Functionality - COMPLETE FIXED VERSION
...
class SearchManager {
    ...
    validatePlateFormat(plate) {
        ...
        // Verificar se tem pelo menos 2 letras e 2 números
    ...
    /**
     * Perform advanced search based on MAC and plate inputs - FIXED
     */
    ...
    /**
     * Display advanced search results with pagination (5 items per page)
     */
    displayAdvancedResults() {
        ...
        // Update results count for advanced search
    ...
    createVehicleElement(vehicle) {
        ...
        div.innerHTML = `
            ...
            <div class="mt-3 flex space-x-2">
                <button onclick="searchManager.editVehicle('${vehicle.mac}')" class="text-blue-600 hover:text-blue-800 text-sm">
                    <i class="fas fa-edit mr-1"></i>Editar
                </button>
                <button onclick="searchManager.removeVehicle('${vehicle.mac}')" class="text-red-600 hover:text-red-800 text-sm">
    ...
    updateAdvancedPagination() {
        ...
        // Update page info for advanced search
        const pageInfoAdvanced = document.getElementById('page-info-advanced');
        if (pageInfoAdvanced) {
            pageInfoAdvanced.textContent = `Página ${this.currentPageAdvanced} de ${totalPages}`;
        }
    }
    ...
    editVehicle(mac) {
        ...
        if (vehicle) {
            ...
            if (macInput) macInput.value = vehicle.mac;
    ...
    removeVehicle(mac) {
        if (confirm('Tem certeza que deseja remover este veículo?')) {
            const index = this.authorizedVehicles.findIndex(v => v.mac === mac);
            if (index > -1) {
                const vehicle = this.authorizedVehicles[index];
                this.authorizedVehicles.splice(index, 1);
                ...
                if (window.addLog) {
                    window.addLog(`Veículo removido: ${vehicle.plate} (${vehicle.mac})`);
                }
            }
        }
    }
    ...
    /**
     * Update metrics by MAC
     */
    updateMetricsByMAC() {
        const macValue = this.macInputMetrics.value;
        const vehicle = this.authorizedVehicles.find(v => v.mac === macValue);
        ...
        if (vehicle) {
            ...
            this.updateMetricsDisplay(vehicle);
        }
    }

    /**
     * Update metrics by plate
     */
    updateMetricsByPlate() {
        const plateValue = this.plateInputMetrics.value;
        const vehicle = this.authorizedVehicles.find(v => v.plate === plateValue);
     ...
     * Update metrics display
     */
    updateMetricsDisplay(vehicle) {
        // This would update charts and metrics based on the selected vehicle
        // Implementation depends on your metrics requirements
        console.log('Updating metrics for vehicle:', vehicle);
    }

    /**
     * Update metrics dropdowns with new vehicle
     */
    updateMetricsDropdowns(mac, plate) {
        // Update MAC datalist
        const macList = document.getElementById('mac-list');
        if (macList) {
            const existingOption = Array.from(macList.options).find(option => option.value === mac);
            if (!existingOption) {
                const option = document.createElement('option');
                option.value = mac;
        ...
        // Update plate datalist
        const plateList = document.getElementById('plate-list');
        if (plateList) {
            const existingOption = Array.from(plateList.options).find(option => option.value === plate);
            if (!existingOption) {
                const option = document.createElement('option');
                option.value = plate;
...
if (typeof module !== 'undefined' && module.exports) {
    ...
    /**
     * Display advanced search results with 5 items per page - FIXED
     */
    displayAdvancedResults() {
