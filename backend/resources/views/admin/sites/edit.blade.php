...
<head>
    ...
    <title>Editar Site - {{ $site->name }}</title>
    <style>
...
<body>
    ...
    <div class="container">
        <h2>Editar Site: {{ $site->name }} (Empresa: {{ $site->company->name }})</h2>
        <form action="{{ route('admin.sites.update', $site) }}" method="POST">
            @method('PUT')
            @include('admin.sites._form', ['submitButtonText' => 'Atualizar Site', 'site' => $site, 'companies' => $companies])
        </form>
    </div>
</body>
</html>
