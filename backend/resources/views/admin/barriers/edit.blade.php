...
<head>
    ...
    <title>Editar Barreira - {{ $barrier->name }}</title>
    <style>
...
<body>
    ...
    <div class="container">
        <h2>Editar Barreira: {{ $barrier->name }} (Site: {{ $barrier->site->name }})</h2>
        <form action="{{ route('admin.barriers.update', $barrier) }}" method="POST">
            @method('PUT')
            @include('admin.barriers._form', ['submitButtonText' => 'Atualizar Barreira', 'barrier' => $barrier, 'sites_grouped' => $sites_grouped])
        </form>
    </div>
</body>
</html>
